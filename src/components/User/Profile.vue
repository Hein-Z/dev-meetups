<template>
  <div>
    <v-row>
      ...
      <v-col cols="12"
        ><h1 class="text-center" style="color: #963E75">
          Created MeetUps By You
        </h1></v-col
      >
    </v-row>
    <v-row>
      <v-col
        xs="12"
        md="6"
        v-for="(item, index) in createdMeetups"
        :key="index"
      >
        <v-card color="#1F7087" dark class="mx-5">
          <div class="d-flex flex-no-wrap  flex-column justify-space-between">
            <div class="d-flex flex-nowrap flex-row justify-space-between">
              <div>
              <v-card-title class="headline" v-text="item.place"></v-card-title>
              <v-card-subtitle
                v-text="item.title"
                class="pb-0"
              ></v-card-subtitle>
              <v-card-subtitle
                v-text="'Create by : ' + item.author_name"
                class="pb-0"
              ></v-card-subtitle>
              <v-card-subtitle
                v-text="'Date : ' + item.date + ' , ' + item.time"
                class="pb-0"
              ></v-card-subtitle>
              </div>
    ...
    <h5 class="text-center" v-if="!createdMeetups.length">
      Meetups You Created Will Be Appear Here
    </h5>
    <v-row>
      <v-col cols="12"
        ><h1 class="text-center" style="color: #963E75">
    ...
    <v-row>
      <v-col xs="12" md="6" v-for="item in registeredMeetups" :key="item.id">
        <v-card color="#EA4863" dark class="mx-5">
    ...
    <h5 v-if="!registeredMeetups.length" class="text-center">
      Meetups You Registered Will Be Appear Here
    </h5>
  </div>
</template>
<script>
