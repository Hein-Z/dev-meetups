    ...
    <v-app>
      ...
      >
        <v-list-item router :to="{ name: 'Home' }" style="color: #6A76AB">
          <v-list-item-content>
            ...
            >
              {{
                isUserAuth
                  ? "User Name : " + $store.getters.getUser.username
                  : "Home"
              }}
            </v-list-item-subtitle>
          </v-list-item-content>
        ...
        <v-list dense nav>
          ...
          <v-list-item link router :to="{ name: 'Signup' }" v-if="!isUserAuth">
            <v-list-item-icon>
          ...
          <v-list-item link router :to="{ name: 'Signin' }" v-if="!isUserAuth">
            <v-list-item-icon>
        ...
        <template v-slot:extension v-if="isUserAuth">
          <v-tabs grow>
            ...
            <v-tab router :to="{ name: 'Profile' }">
              ...
              <v-badge color="green" :content="$store.getters.getNotifications">
      ...
      <v-sheet>
        ...
        > -->
          <router-view style="margin-top:110px;"></router-view>
        <!-- </v-row> -->
      </v-sheet>
    </v-app>
  </div>
...
export default {
  data: () => ({
    ...
    swipeDirection: 'None',
    count: 0,
    route: ['Home', 'Meetups', 'Profile', 'CreateMeetup'],
    items: [
      {
        title: 'View Meetups',
  ...
  methods: {
    ...
    // swipe (direction) {
    //   if (this.$route.name === 'MeetupDetails') {
    //     return false
    //   }
    ...
    //     this.route.unshift(this.route[this.route.length - 1])
    //     this.route.pop()
    //   }

    //   if (this.isUserAuth) {
    //     this.$router.push({ name: this.route[0] })
    //   }
    // }
  },
  created () {
